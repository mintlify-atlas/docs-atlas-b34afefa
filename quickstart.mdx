...
description: "Go from zero to a fully enriched postal code result in minutes — decode GPS coordinates, validate format and geography, run the CLI, and resolve estafeta and political divisions."
---
...
<Steps>
  ...
  print(d.lat, d.lng)          # 8.94444609375  -79.56167109375001
  print(d.level)               # PICO
  ...
  By default, `validate()` uses Panama's strict territorial bounding box (`lat ∈ [7.15, 9.70]`, `lng ∈ [-83.10, -77.10]`). Pass `strict=False` (or use the `--laxo` CLI flag) to switch to the official site's permissive bounding box, which includes adjacent ocean areas.
